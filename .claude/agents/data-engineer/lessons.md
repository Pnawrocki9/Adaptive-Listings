...
- **2026-08-23 / FOLLOW-853** · Fixed the ClickHouse timestamp drift: both ingest writers now emit
  `YYYY-MM-DD hh:mm:ss.mmm` via one canonical `toClickHouseDateTime64`, plus a live-engine
  integration spec (happy path / negative control / prod-parser parity) in the `clickhouse-smoke` CI
  job, a smoke fixture that finally sends the writer's real bytes, and the AC(4) prod-setting
  runbook. · **Risks weighed:** the dispatch brief told me to apply the encoder to
  `intent-snapshot.ts:294-295` too — reading HEAD showed those lines are a _Supabase/PostgREST_
  writer where the trailing `Z` is REQUIRED, so following the brief would have regressed the
  intent-session upsert. Also rejected the "just add `date_time_input_format=best_effort` to the
  URL" option: it trades a dependency on the server DEFAULT for one on the profile's PERMISSION to
  override it (Code 452), which is unknown because the prod probe never captured `currentUser()`.
  Chose the byte shape that is the INTERSECTION of both parser modes, and discharged the
  prod-regression risk with an actual `best_effort` test case instead of an argument. · **Guardrail
  I'd add:** _a fixture that does not send the production writer's exact bytes is not coverage._
  `smoke-test.sh` inserted an unquoted numeric epoch for months — a shape no writer ever emits — so
  CI was structurally incapable of catching a total, silent, post-ACK write loss. Proposed rule: any
  CI fixture standing in for a production writer must be generated by, or byte-compared against,
