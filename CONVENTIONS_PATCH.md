...
## Rule AA — AC deliverability: an AC naming a durable queue job or external-system call requires the real implementation, not a log/Sentry stub

**Pattern:** A ticket's acceptance criteria name a specific durable external target — a Modal task
queue, a Redpanda enqueue, a durable RPC — and the worker implements a logging-only substitute (a
`Sentry.captureMessage` call, a `console.warn`, a TODO comment pointing at the real target) and
marks the ticket DONE. The logging stub is observable (better than a silent drop) but is NOT AC
fulfillment: the chartered durable path is not connected, and any overflow or event the AC says
"enqueued" is silently lost after the log drains.

**Evidence (≥2 prior retros):**

- **RETRO-141 §4a LG-1 (count 1 — held)** — FOLLOW-434 AC-1 said "overflow is enqueued to a
  Modal/queue job." The shipped implementation replaced the enqueue with
  `Sentry.captureMessage(…) + console.warn` and a
  `TODO FOLLOW-434 — replace overflow stub with Modal job`. Recorded in RETRO-141 §6 as "AC-STUB vs
  AC-DONE, count 1 — held."
- **RETRO-142 §6 (count 2 — promoting)** — FOLLOW-435 closed the RETRO-141 LG-1 gap by shipping the
  real Redpanda producer (LEG 1) and Modal consumer (LEG 2). Both independently reached CI-green and
  AC-met state. Confirming the sub-pattern: the only reason FOLLOW-435 existed at all was that
  FOLLOW-434 delivered a stub instead of the real enqueue. Count 2, non-adjacent retros.

**Rule:** If an acceptance criterion names a durable external system call (Modal task queue,
Redpanda topic enqueue, Postgres write, Redis set, durable RPC), the implementation in the PR that
claims AC fulfillment MUST wire the real call. A Sentry/log stub with a TODO pointing at the real
target is NOT AC fulfillment — it is a scope reduction. The correct procedure is:

1. If the real implementation genuinely cannot land in the current ticket (dependency not ready,
   infra not provisioned), file the scope reduction BEFORE marking DONE: add a `FOLLOW-NNN` stub in
   `backlog/FOLLOW_UPS.md` AND update the ticket's AC list in the spec to document the deferral
   explicitly.
2. Mark the ticket DONE only for the reduced scope (log stub), note the residual in `notes:`, and
   leave the remaining AC as an unchecked item pointing to the FOLLOW.
3. The downstream FOLLOW that wires the real call inherits the original AC wording and must be
   marked DONE only when the wire is present and CI-green.

A "DONE" ticket with an open AC that said "enqueue to X" but delivered "log to Sentry" is a
HALF_WIRE: an operator cannot observe completion, retries are manual, and the gap is invisible to
monitoring. Escalate scope reductions to PM before marking DONE.

**Verification:**

```bash
# Before marking a ticket DONE on a PR that contains a TODO pointing at a queue/RPC target,
# grep for any Sentry-only stub adjacent to a TODO naming a deferred system call:
grep -rn "TODO.*Modal\|TODO.*queue\|TODO.*Redpanda\|TODO.*enqueue" apps/ packages/ \
  --include="*.ts" --include="*.py" | grep -v node_modules | grep -v "\.test\."
# Each hit is a potential AC-STUB. Confirm: is the adjacent AC item checked? If so, AC is
# mis-checked — bounce to IN_PROGRESS until the real call is wired.
```

---

<!-- Rule AA added 2026-06-30 — RETRO-142 §6 (AC-STUB vs AC-DONE pattern: RETRO-141 §4a LG-1 FOLLOW-434 Sentry-stub delivered where AC said "enqueued to Modal queue", count 1 held; RETRO-142 §6 confirming FOLLOW-435 existence was itself caused by the FOLLOW-434 stub, count 2; ≥2 non-adjacent retros met threshold). DISTINCT axis from Rule H (wired-or-dead, gates source-importers at PR level) and Rule M (merged-not-live for migrations/deploys): Rule AA governs CLAIMED AC fulfillment at ticket DONE time — a stub with a TODO is not a wired consumer and is not a DONE AC. Anti-count-inflation honored (counts came from two prior retros; the promoting retro RETRO-142 does not inflate the count). Filed FOLLOW-436 (operator go-live: Modal secrets + redeploy, the Rule AA-compliant closure of the deployment gap). Letter choice: single letters A–Z exhausted; AA is the next available designation after the single-letter space. -->
