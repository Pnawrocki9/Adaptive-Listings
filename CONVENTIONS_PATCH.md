...
### Rule AZ amendment 1 (2026-09-14 — RETRO-329 §6 Candidate L — when a SIBLING PR lands first, the merging actor re-reads this branch's sentences about it and that sibling's handoffs to this ticket, against the FINAL base, before merging)

**This is an AMENDMENT to an already-promoted rule, not a new rule.** No letter is minted. RETRO-327
§6 pre-committed the home before this evidence existed (_"at 2 PRIOR, amend Rule AZ (the findings
grep is run against the FINAL base, after rebasing onto any sibling that merged first, not at
drafting time) rather than mint a letter"_), so the amendment discharges a falsifiable prediction.

- **What Rule AZ covers today.** Clause 1 fires when a section is regenerated. Clause 3 covers a
  ruling that lands first.
- **What neither reaches.** Two PRs are drafted in parallel, and each writes about the other: "once
  X lands", "not on `main` yet", a handoff addressed to the other ticket's agent. Then one merges
  first.
- **Rebasing does not fix it.** The branch absorbs the sibling's CODE, and the sentences and
  handoffs about it stay as drafted.

**Evidence (≥2 PRIOR numbered retros; the promoting pair RETRO-329/330 is one PR and is not
counted):**

- **RETRO-302 §4a LG-2 (sighting).** #829 merged 8 seconds after #828. Nothing carried the triage's
  correction forward into the files #828 touched.
- **RETRO-327 §6 (count 2, instances 1–5).** In the #899/#900/#901 train:
  - two retro amendments landed 15 and 30 seconds after the tickets they amended had closed;
  - #899 and #900 each deferred to the other;
  - #902's harness handoff sat in unmerged ESC-079 text at pre-#899 anchors.
- **RETRO-328 §6 (instance 6).** #900's SoT describes FOLLOW-1196 as pending in five status
  sentences. FOLLOW-1196 had merged 15 seconds earlier.
- **RETRO-329 / RETRO-330 (promoting, three instances):**
  1. **The canary handoff.** #902's handoff "qa-engineer: run the canary with the ops bearer, or
     accept the 10%" was homed onto FOLLOW-1207 AC(4) and died when #904 merged without it.
     **Measured consequence:** a registered required gate went red at random (run 34787084634).
  2. **A docblock.** `harness-preflight.test.ts` explains a missing real-handler test with "cannot
     import a handler that is not on `main` yet". #902 had merged eight hours before #904, and the
     PM had merged `origin/main` into #904's branch and re-run its suite.
  3. **A runbook line.** README `:366`, "once FOLLOW-1201 lands", merged after FOLLOW-1201 landed.

**Amendment (adds clause 5 to Rule AZ):**

5. **When a branch absorbs a sibling PR that merged after the branch was cut** (by rebase or by
   merging `origin/main`), the actor who merges it (the PM when the author has finished) runs two
   greps against the FINAL base before merging, and records both in the PR body:
   - **(a) This branch's sentences about the sibling.** Grep the branch's own diff for the sibling's
     ticket id and PR number, and for conditional phrasing ("once … lands", "not on `main` yet",
     "until …", "before #N", "after #N merges"). Rewrite each one the sibling's merge made false.
     Keep each one that stays true, and say that it stays true.
   - **(b) The sibling's handoffs to this branch.** Grep the sibling's PR body, its escalation text
     and the backlog lines it added for this branch's ticket id, touched file paths and owning
     agent. Discharge each hit in this PR, or re-home it BY NAME onto a ticket that is still open
     after both merge (Rule AW's shape). A handoff addressed to an agent rather than a ticket
     counts.
6. **A retro that finds a clause-5 miss reports the handoff's full path** (who wrote it, where it
   was homed, which merge dropped it), not only the stale sentence. The path is what tells the next
   dispatcher where to look.

**Verification:**

```bash
# 1. Siblings that merged after this branch was cut:
git log --format='%h %s' "$(git merge-base origin/main HEAD)"..origin/main
# 2. (a) this branch's own sentences about them:
git diff origin/main...HEAD | grep -nE '^\+.*(FOLLOW-<sibling>|#<sibling-pr>|once [^ ]+ lands|not on .?main.? yet|until (FOLLOW|#)[-0-9]+|before #[0-9]+)'
# 3. (b) the sibling's handoffs addressed here (ticket id, file path, agent name):
gh pr view <sibling-pr> --json body --jq .body | grep -nE '<this-ticket>|<touched-file>|<agent>'
git diff "$(git merge-base origin/main HEAD)"..origin/main -- backlog/ | grep -nE '^\+.*(<this-ticket>|<touched-file>)'
```

**The negative case, so the amendment is falsifiable:** a branch that absorbed no sibling merged
after its cut owes nothing under clause 5, and neither does a sibling that says nothing about this
ticket, file or agent. The amendment fires on the merge-train topology only.

**Distinct from:** clause 3 (a RULING lands first; here it is a peer PR, and the stale text is often
about code, not a decision); Rule AW (a ticket's `blocks:` field at its own closure; here the
orphaned item is a prose handoff that was never a `blocks:` entry); Rule AX (perishable line
anchors; here the whole sentence is false, whatever its anchor); Rule BA (work on `origin`; here
