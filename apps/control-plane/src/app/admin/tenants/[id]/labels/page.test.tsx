/**
 * Tests for /admin/tenants/[id]/labels (staff per-tenant labels editor, FOLLOW-597).
 *
 ...
 *   - renders the staff {@link StaffLabelsEditor} with `tenantId={id}` on a known tenant.
 *
 ...
 *
 * @module apps/control-plane/src/app/admin/tenants/[id]/labels/page.test
 */
...
vi.mock('./labels-editor', () => ({
  StaffLabelsEditor: ({ tenantId }: { tenantId?: string }) => (
    <div data-testid="labels-editor">tenant:{tenantId ?? 'none'}</div>
  ),
}));

import { tenantExists } from '@/lib/session-auth';
import { notFound } from 'next/navigation';
import StaffTenantLabelsPage from './page';
...
describe('StaffTenantLabelsPage', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('renders notFound() (404) when the tenant does not exist', async () => {
    ...
    await expect(
      StaffTenantLabelsPage({ params: Promise.resolve({ id: UNKNOWN_TENANT }) }),
  ...
  it('renders StaffLabelsEditor with tenantId on a known tenant', async () => {
    mockTenantExists.mockResolvedValue(true);

    const ui = await StaffTenantLabelsPage({
    ...
    const editor = screen.getByTestId('labels-editor');
