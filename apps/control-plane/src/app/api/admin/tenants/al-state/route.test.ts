/**
 * FOLLOW-633 — tests for GET + PUT /api/admin/tenants/al-state (the audited,
 * staff-only write path for the per-tenant Adaptive Listings on/off).
 *
 ...
 * shape and observe the tenant fence.
 *
 * Coverage:
 *   - Staff can flip al_enabled; the write is atomic (column + audit row in ONE tx).
 *   - The staff_audit_log row is written with action 'tenant_al_state.update' + delta.
 *   - Agency session → 403 staff_only (no agency write path here).
 ...
 *
 * @module apps/control-plane/src/app/api/admin/tenants/al-state/route.test
 */

import { NextRequest } from 'next/server';
import { describe, expect, it, vi, beforeEach } from 'vitest';
import type * as SessionAuthModule from '@/lib/session-auth';
import type { TenantAlState } from './route';
...
function makeRequest(opts?: {
  ...
  const url = new URL('http://localhost/api/admin/tenants/al-state');
...
describe('GET /api/admin/tenants/al-state', () => {
  it('staff → 200 with the tenant al_enabled + status', async () => {
    mockResolve.mockResolvedValue(staffAccess(TENANT_A));
    makeDb({ [TENANT_A]: { alEnabled: false, status: 'active' } });

    const res = await GET(makeRequest({ query: { tenant_id: TENANT_A } }));
    expect(res.status).toBe(200);
    const body = (await res.json()) as TenantAlState;
    expect(body.al_enabled).toBe(false);
...
describe('PUT /api/admin/tenants/al-state', () => {
  it('staff ops → flips al_enabled AND writes an atomic staff_audit_log row', async () => {
    mockResolve.mockResolvedValue(staffAccess(TENANT_A));
    const db = makeDb({ [TENANT_A]: { alEnabled: true, status: 'active' } });

    const res = await PUT(
      makeRequest({ method: 'PUT', query: { tenant_id: TENANT_A }, body: { al_enabled: false } }),
    );
    expect(res.status).toBe(200);
    const body = (await res.json()) as TenantAlState;
    expect(body.al_enabled).toBe(false);
    ...
    expect(db._tenants[TENANT_A]!.alEnabled).toBe(false);
    ...
    const audit = db._auditRows[0] as {
      ...
      payload: { before: { al_enabled: boolean }; after: { al_enabled: boolean } };
    };
    expect(audit.action).toBe('tenant_al_state.update');
    expect(audit.adminUserId).toBe('staff-uuid-777');
    expect(audit.targetTenantId).toBe(TENANT_A);
    expect(audit.payload.before.al_enabled).toBe(true);
    expect(audit.payload.after.al_enabled).toBe(false);
  ...
  it('below-ops staff (readonly) → 403 and NO mutation / NO audit row', async () => {
    ...
    expect(res.status).toBe(403);
    expect(db._tenants[TENANT_A]!.alEnabled).toBe(true); // untouched
    expect(db._auditRows).toHaveLength(0);
    expect(db.transaction).not.toHaveBeenCalled();
  });

  it('agency → 403 staff_only (no agency write path)', async () => {
    mockResolve.mockResolvedValue(agencyAccess(TENANT_A));
    const db = makeDb({ [TENANT_A]: { alEnabled: true, status: 'active' } });
    const res = await PUT(
      makeRequest({ method: 'PUT', query: { tenant_id: TENANT_A }, body: { al_enabled: false } }),
    ...
    const body = (await res.json()) as { error: { code: string } };
    expect(body.error.code).toBe('staff_only');
    expect(db._tenants[TENANT_A]!.alEnabled).toBe(true);
  });

  it('invalid body (missing al_enabled) → 400', async () => {
    mockResolve.mockResolvedValue(staffAccess(TENANT_A));
    makeDb({ [TENANT_A]: {} });
    const res = await PUT(
  ...
  it('tenant fence (invariant 5): the write targets ONLY the resolved tenant', async () => {
    mockResolve.mockResolvedValue(staffAccess(TENANT_A));
    const db = makeDb({
      [TENANT_A]: { alEnabled: true, status: 'active' },
      [TENANT_B]: { alEnabled: true, status: 'active' },
    });
    ...
    expect(db._tenants[TENANT_B]!.alEnabled).toBe(true); // sibling tenant untouched
  });
});
