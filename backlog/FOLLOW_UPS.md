...
## FOLLOW-475 — Durable, crash-survivable retry queue for the post-ACK ClickHouse insert (Cloudflare Queues)

- **source_retro:** none yet (filed inline by the FOLLOW-459 implementation, per the ticket's own
  AC2: "if a durable retry is not feasible without new infrastructure, capture-to-Sentry + a clear
  code comment + a follow-up note is acceptable")
- **source_ticket:** FOLLOW-459 — F-09 (2026-07-01 audit) — ACK-before-ClickHouse-insert
- **recommended_sprint:** next available (P2 — closes an observability-only gap, not a go-live
  blocker; the write itself already retries 3x with backoff before this gap is even reached)
- **recommended_agent:** backend-engineer (Cloudflare Queues is new infra for `apps/ingest` — an
  ADR + `docs/ESCALATIONS.md`-class sign-off is needed before adding the binding, per CLAUDE.md's
  "new third-party service" escalation rule; Cloudflare Queues is same-vendor as the rest of the
  Worker stack, but it is still a NEW binding/cost surface, not something already wired for ingest)
- **priority:** P2
- **estimated_hours:** 6
- **scope:** FOLLOW-459 moved the ClickHouse `events` insert off the ACK critical path via
  `ctx.waitUntil()`, preserving its existing 3-attempt/backoff in-process retry policy. A terminal
  failure (all 3 attempts exhausted, or a 4xx) after that point is captured to Sentry (`events.ts`,
  tags `{ area: 'events', sink: 'clickhouse', kind: 'insert_failed' }`) but the batch itself is NOT
  re-queued anywhere — if ClickHouse is down for longer than the in-process retry window (~3.1s),
  those events are permanently lost from ClickHouse (they DID reach Redpanda, so they are not lost
  ...
  own backoff, so failed batches survive past the Worker's own request lifecycle instead of only
  being observable via Sentry.
- **ac:**
  - [ ] ADR proposal (`docs/adr/PROPOSED-XXX.md`) for adding Cloudflare Queues to `apps/ingest`,
        including cost estimate (must be escalated if >€100/mo per CLAUDE.md)
