...
- KV namespaces: API_KEYS=`523aafacf2d54201a33631d62ba801e3`, IDEMPOTENCY=`0603c2833a7a46e2890fb885319d813c`
...
- **WAŻNE — NPM_CONFIG_PRODUCTION=false** w Vercel env vars (Production + Preview) — bez tego Vercel pomija devDependencies i build failuje
- Install Command override: `cd ../.. && pnpm install --frozen-lockfile --prod=false`

**Build fixes (commity na main):**
| Commit | Fix |
|--------|-----|
...
**Step 5: Smoke Test ✅**
Pipeline end-to-end verified — `accepted:1, rejected:0`:
```bash
...
1. **Zresetuj hasło Supabase DB** — hasło `OliLeo4435OliLeo4435` było widoczne w terminal history. Idź do Supabase Dashboard → Settings → Database → Reset password. Potem zaktualizuj DATABASE_URL i DATABASE_URL_DIRECT w Doppler.
...
- Hooki już gotowe: `container_selector`, `data_extractors_per_card`, `ReorderDirective` type stub w `packages/shared/src/directives.ts`
...
**CI:** GitHub Actions, wszystkie joby zielone. Corpus CI gate (`pnpm test:corpus`) blokuje merge jeśli precision <95%.
...
- Install Command override: `cd ../.. && pnpm install --frozen-lockfile --prod=false`
- Turbo wykrywa się automatycznie → Build Command: `cd ../.. && pnpm turbo run build --filter=@estalara/control-plane`
...
- Git user.email MUSI być `piotr@time2show.com` (GitHub primary email) — inaczej Vercel blokuje deploy
...
*Wygenerowano: 2026-05-19 | Sesja: Phase 1 Vendor Activation Complete — smoke test passed*
*Poprzedni HANDOFF: v3.0 (2026-05-13)*
