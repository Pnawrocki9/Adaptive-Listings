...
- **FOLLOW-105 Wave 1 (substeps 1b + 1c + 1d) — IN_PROGRESS (since 2026-05-25).** Single
  comprehensive PR. Branch: `feat/follow-105-1bcd-canonical-adapt-enforcement`. Lead: architect
  (opus-4.7-xhigh); support: backend-engineer (Worker 410 Gone), sdk-engineer (buildSnippet + SDK
  Zod). Scope: buildSnippet fix (F.1), demo mockup fix (F.2), DECISION_API_URL deprecation (F.3),
  Worker 410 Gone + structured logging (F.4), SDK Zod validation (F.5), adapt_decision_id UUID (F.6
  partial). ADR-0006 → ACCEPTED + ADR-0004 contract block replaced (live wins) on merge.
- **Wave 2 (FOLLOW-097 + FOLLOW-106) — BLOCKED** until Wave 1 PR merges.
...
_No PRs in flight._ Wave 1 PR (`feat/follow-105-1bcd-canonical-adapt-enforcement`) opens on worker
completion; PM validates against acceptance criteria + waits for CI green (`gh pr checks --watch`)
