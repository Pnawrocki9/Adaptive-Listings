  ...
  plugins: [
    {
      rules: {
        ...
        'ticket-reference': (parsed) => {
          ...
          // [TICKET-AB-NNN], or [ESCALATION] reference
          const ticketPattern =
            /\[TICKET-(?:FIX-|INFRA-|DEMO-|ADM-|QUIZ-|DB-|EMB-|ARCH-|ADP-|DQS-|AUTO-|AB-)?\d+\]|\[ESCALATION\]/;
          if (!ticketPattern.test(subject)) {
            return [
              false,
              'Commit message must include [TICKET-XXX], [TICKET-FIX-XXX], [TICKET-INFRA-XXX], [TICKET-DEMO-XXX], [TICKET-ADM-XXX], [TICKET-QUIZ-XXX], [TICKET-DB-XXX], [TICKET-EMB-XXX], [TICKET-ARCH-XXX], [TICKET-ADP-XXX], [TICKET-DQS-XXX], [TICKET-AUTO-XXX], [TICKET-AB-XXX], or [ESCALATION] reference. Example: feat(ingest): add validation [TICKET-042]',
            ];
          }
