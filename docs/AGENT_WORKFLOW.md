...
This repo runs an **agent-orchestrated** development workflow. Most code is written by specialized Claude Code subagents under the supervision of a PM agent. Humans (Piotr, Rafał, Krystian) review PRs and resolve escalations.

Critical constraint from Claude Code: **subagents cannot talk to each other directly**. They only communicate through files in this repo. The orchestration is built around that constraint.
...
These files are the single source of truth. Agents read and write them; humans read and occasionally edit.
...
The PM agent runs autonomously between your sessions, picking up tickets and validating PRs. When you return, you primarily process its output (review + merge + escalations) rather than writing prompts.
...
Two agents shouldn't have been picking those tickets. PM should have detected the conflict via `produces`/`affects` lists in tickets. If it didn't:
- Close the later PR
...
Generally don't. The 9 we have cover MVP scope. If a recurring need appears that doesn't fit any agent:
...
- Not autonomous AGI building your product. You stay in the loop on architecture, merges, and escalations.
- Not a replacement for code review. PM validates AC; humans validate quality and judgment.
- Not magic. Agents fail. Loops break. Have backups (git, Doppler) and don't push agents to production directly.
