...
| `scoring_path`        | `LowCardinality(String)` | `'not_applicable'` | 0022  | FOLLOW-560. Which mechanism ranked this decision's `ReorderDirective`. Values: `'cosine'` (every listing scored by cosine(archetype_embedding, listing_embedding)), `'djb2_fallback'` (embeddings were fetched but at least one listing fell back to the djb2 stable hash), `'djb2_guard'` (embeddings never attempted — `listing_ids` over `LISTING_EMBEDDING_BATCH_LIMIT`, or the lookup threw — so the whole batch used djb2), `'not_applicable'` (no reorder was built: GET /api/adapt, no `listing_ids`, or a tenant schema that is not `reorder_capable`). Writer: `logDecisionAsync()` in `apps/control-plane/src/app/api/adapt/route.ts`, which emits the column **only when `SCORING_PATH_COLUMN_ENABLED=true`** (ESC-031 safety — migration 0022's prod apply is deferred to FOLLOW-820, so the default INSERT omits the column entirely and rows written before the flag is flipped read the DEFAULT). Consumer: FOLLOW-819's differentiator E2E, which uses it to tell a real personalized ranking from a session-stable shuffle. |
