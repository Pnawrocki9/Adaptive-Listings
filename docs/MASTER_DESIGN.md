...
**Wersja:** 4.8 (2026-08-08 — §Snapshot.1 wiersz B.6: deployed artefact nie startował [FOLLOW-900]; `deployed` ≠ `running` ≠ `importable`. Poprzednio: 4.7 (2026-08-07) — staging-plane retirement + §Snapshot.1 deploy-state reconciliation [FOLLOW-878 + FOLLOW-891]: §V.6.1/§V.6.3 przestają opisywać environment `staging`, który nigdy nie istniał jako izolacja (ESC-052 RESOLVED, CEO option 2 — `stg.DATABASE_URL_ADMIN` byte-identyczny z `prd`); §Snapshot.1 wiersze A.1/B.6/D skorygowane przeciwko świeżo wykonanemu `modal app list` i sondzie rejestracji funkcji — status i flip-condition wiersza B.6 NIETKNIĘTE)
