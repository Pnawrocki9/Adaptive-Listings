...
| `fallback_reason`   | `'llm_unavailable' \| 'fact_check_refused'` (optional)                                                                            | FOLLOW-1056. Present only on a `playbook_fallback_*` response; says which of two OPPOSITE conditions produced it — the LLM never produced usable output (an incident), or it did and the FOLLOW-457 fact check refused to serve it (the system working). `source` cannot carry the split: it is a strict `z.enum` in the SDK schema, so a new value there fails validation in every deployed bundle and drops the whole response. Diagnostic only — the SDK's behaviour is identical for both values. |
