...
| [ADR-0020-shadow-intent-write-admission](ADR-0020-shadow-intent-write-admission.md)                                   | ACCEPTED | 2026-07-30 | Shadow chat-intent key write-admission: an extraction carrying no usable dimensions (degraded OR neutral-success) is written with `SET … NX` and therefore never clobbers, and never TTL-refreshes, an accumulated prior; keyed on content not `data_source`; no read-back, so `payload.model_dump()` stays the only write path; no merged record, so provenance markers always describe the dims beside them. Implementation: FOLLOW-736.                                                                                                                                 |
| [ADR-0021-consent-text-out-of-bundle-transport](ADR-0021-consent-text-out-of-bundle-transport.md)                     | PROPOSED | 2026-08-08 | ESC-051/FOLLOW-915: consent-banner text moves out of the SDK bundle into one identifier-free static JSON asset (`GET {CONTROL_PLANE_URL}/consent-text.json`, no auth/params/cookies, all locales in one doc), fetched pre-consent on the `pending` path only; banner awaits it; fail-closed on error (no banner, zero events, no fallback text). Scope-clarifies ADR-0011 addendum: tenant-data-after-consent re-affirmed; "banner cannot wait for the fetch" scoped to `fetchQuizConfig()`. ACCEPTED on merge; compliance countersign on §D5 gates the implementation PR. |
