...
**Version:** 1.8 **Date:** 2026-08-24 **Author:** Compliance Engineering **Regulatory basis:** GDPR
...
> **⚠ DO NOT PUBLISH §2 YET — the 7-day retention it promises is not enforced (FOLLOW-140 /
> ESC-071).** Grep-verified 2026-08-24: the `consent.denied` audit event travels the ordinary ingest
> path into ClickHouse `events`, whose only TTL is `toDateTime(ts) + INTERVAL 13 MONTH`
> (`infra/clickhouse/migrations/0001_create_events.sql:46`). No 7-day sweep exists — the three
...
> consent-audit rows. The paragraph below therefore states a retention period the product does not
> currently apply, which is the same defect class as the §1 sentence corrected in v1.8, and it is
> **already being told to visitors** by the shipped consent banner in EN, PL and ES. Escalated as
> ESC-071 because the remedy is a choice — enforce a 7-day TTL, or re-word a byte-locked disclosure
> sentence that carries a compliance sign-off — and not an edit this template may make on its own.
> _Kept as written rather than silently softened: the byte-locked banner text
> (`docs/compliance/consent-disclosures.canonical.json`) says "7 days", and this template must not
> diverge from it while the question is open._
...
| **ESC-071 resolved** — §2's 7-day denial-log retention is UNENFORCED (FOLLOW-140) and §2 must not be published until it is enforced or the disclosure is re-worded through the sign-off path                                                                                                       | Compliance Engineering + CEO | PENDING |
