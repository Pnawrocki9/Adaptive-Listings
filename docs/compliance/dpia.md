...
**Document ID:** ESTALARA-DPIA-001 **Version:** 2.13 **Date:** 2026-08-05 **Authors:** Time2Show,
...
| `apps/ingest` (Cloudflare Worker)                                               | **None** — no `beforeSend` (`observability.ts:71-82`). Redaction is at the CALL SITE instead; see §2.7.2       | —                                                          |
...
- **One site outside this ticket's fix does carry the same shape as the one fixed:**
  ...
  dispatch path remains reachable through the ClickHouse batch path.
...
3. `apps/ingest/src/observability.ts` gains an `integrations` or `defaultIntegrations` key, or
   `sendDefaultPii: true` / `includeLocalVariables: true`. Note `sendDefaultPii` is **not** pinned
   explicitly here the way the three control-plane configs pin it (see the FOLLOW-845 note in
   `ropa.md`); the SDK default is falsy, and `@sentry/cloudflare@10.50.0` `build/cjs/sdk.js:14`
   reads `options.sendDefaultPii ?? false`.
