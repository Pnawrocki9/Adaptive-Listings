...
3. The tenant submits a DSR request to the Estalara DSR endpoint — **`POST /api/dsr/initiate`**
   ...
   the requester identifier (session_id, fingerprint_hash, or hashed email if the tenant maintains
   an account link). **[CORRECTED 2026-08-07 — FOLLOW-710 AC-6 / FOLLOW-815, Rule AH.** This step
   named `POST /api/v1/dsr/request` from this document's creation until that date. **No such route
   has ever existed in the repo** (`find apps -ipath "*v1/dsr*"` returns nothing) — so the
   documented intake procedure named a phantom endpoint, and three consecutive retros (RETRO-228 §4a
   LG-2 among them) re-derived the same finding. The real route is and always was
