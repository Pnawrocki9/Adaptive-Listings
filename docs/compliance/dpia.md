...
**Data classification**: Visitor identity is represented solely by a **randomly minted UUID v4**
that is derived from no device, browser or visitor attribute and is held in tab-scoped
`sessionStorage` for the lifetime of the browsing tab. The full mechanism, and precisely what it
does and does not guarantee, is in **§2.2.1** — read that section before relying on any property of
the identifier asserted elsewhere in this document. The system never reads cookies, never stores
...
subjects on a publicly accessible area (real estate listing websites) on a large scale. The product
does **not** employ a device-fingerprinting technique: it reads no canvas, no AudioContext, no WebGL
renderer, no screen geometry, no timezone and no browser language in order to identify a visitor
(§2.2.1; pinned by the SDK test _"does not read navigator.userAgent, screen or Intl"_). What engages
Article 5(3) of Directive 2002/58/EC (ePrivacy Directive) is the storage step itself: the SDK
**writes** the session identifier into `sessionStorage` and **reads** it back on every subsequent
page load in the tab, together with the `localStorage` keys enumerated in
`PRIVACY_NOTICE_TEMPLATE.md` §4. Writing to and reading from a visitor's terminal equipment is
"storing information, or gaining access to information already stored" within the meaning of Article
5(3) irrespective of the technique used — EDPB Guidelines 02/2023 on the technical scope of Art.
5(3) of the ePrivacy Directive (adopted 14 November 2023, finalised October 2024) are explicit that
the provision is technology-neutral and reaches local storage and fingerprinting alike, not only
cookies. The processing is also processing of personal data within the meaning of the GDPR, because
a session-scoped pseudonymous identifier constitutes personal data under Article 4(1) when
associated with online behavior. **The Article 35(3)(c) trigger below does not depend on
fingerprinting** — it rests on systematic monitoring of behavior on a publicly accessible area,
which the product does regardless of how the identifier is minted.
...
- **Mode A — Session Mode (default):** The session identifier is a randomly minted UUID v4 held in
  tab-scoped `sessionStorage`; it is derived from nothing about the device, the visitor or the
  tenant, and it is not recomputed — see **§2.2.1** for the mechanism and for the bounded statement
  of what that does and does not preclude. This mode is _intended_ to rely on the ePrivacy Directive
  Article 5(3)(b) "strictly necessary" exemption for services explicitly requested by the user;
  whether it does so is a question for counsel and is recorded as open in §4 Risk E and in
  `docs/compliance/FOLLOW-1105-session-identifier-assessment.md` §9. Data is scoped to one session
  on one tenant site. **Correction (v2.20, FOLLOW-1107):** this bullet previously read _"It does not
  require a consent banner"_. The shipped SDK is more conservative than that sentence: when the
  stored consent state is `pending` it renders the banner and awaits a decision before any session
  is created, and it fails closed if the consent text cannot be fetched (`packages/sdk/src/index.ts`
  — the `consentState === 'pending'` branch gates `getOrCreateSession()`). The carve-out is
  withdrawn from this record rather than retained unexercised; FOLLOW-927 tracked this exact
  divergence and is discharged by this edit.
- **Mode B — Consented Mode:** Tenant collects explicit user consent via the Estalara Consent Helper
  or their own CMP, and passes a verified consent record to the Estalara backend via API. The
  ninety-day cross-session identifier `__estalara_xid__` is enabled — a random UUID in
  `localStorage`, **not** a behavioral fingerprint (§13.2 carries the mechanism and its current
  transmission gap). Cross-listing journeys within a declared partner group are permitted. Sessions
  in Mode B contribute to global archetype training (with differential privacy protections applied).
...
#### 2.2.1 The session identifier — mechanism of record

_This subsection is the single anchor for every claim this document makes about the session
identifier. It is a sub-section of §2.2, not a new top-level section (Rule AN). Added v2.20
(FOLLOW-1107) after ESC-070 Path C shipped as FOLLOW-1106 / PR #844, merged `d9160da0`._

**What is minted.** `generateSessionId()` (`packages/sdk/src/core/session.ts`) returns a UUID v4
drawn from the browser's cryptographic random number generator: `crypto.randomUUID()` where it is
available, and an RFC-4122-shaped value built from `crypto.getRandomValues(new Uint8Array(16))`
where it is not (a plain-HTTP page, where `crypto.randomUUID()` is secure-context gated). If neither
API exists the function **rejects** — the SDK mints no identifier at all rather than a guessable or
device-derived one.

**What is stored, and where.** `getOrCreateSession()` reads `sessionStorage['__estalara_session__']`
**first** and calls `generateSessionId()` only on a miss. `sessionStorage` is scoped to a single
browser tab and is cleared by the browser when that tab is closed. Intra-session stability —
surviving navigation and full page reload — is therefore a property of that storage read, not of the
derivation. Constants: `SESSION_STORAGE_KEY = '__estalara_session__'`; the disclosure row is
`PRIVACY_NOTICE_TEMPLATE.md` §4.

**How to re-check every sentence above.** These properties are pinned by tests in
`packages/sdk/src/__tests__/session.test.ts`, each of which fails if the property is lost:

| Property                               | Test that pins it                                                                     |
| -------------------------------------- | ------------------------------------------------------------------------------------- |
| Randomly minted, not derived           | _"mints 500 distinct ids in one identical environment"_                               |
| No digest is computed at all           | _"never hashes anything — crypto.subtle.digest is not called"_                        |
| No device attribute is read            | _"does not read navigator.userAgent, screen or Intl"_                                 |
| Works without a secure context         | _"falls back to crypto.getRandomValues when randomUUID is absent (insecure context)"_ |
| No weak fallback when no CSPRNG exists | _"rejects loudly rather than minting a weak or device-derived id"_                    |

**What this guarantees.** Because the identifier contains no input derived from the device, the
visitor, the tenant or any earlier session, it cannot be **re-derived**: no party, including
Time2Show, can recompute a visitor's earlier session identifier from their browser, and two
identifiers minted in two tabs or on two visits carry no computable relation to each other. Two
distinct visitors also cannot be assigned the same identifier by sharing a device profile, which the
previous mechanism did.

**What this does not guarantee, stated deliberately.** This is a property of the identifier only. It
is **not** a claim that two visits by the same person can never be associated. The residual linkage
vectors present at `d9160da0`, each assessed in its own section, are:

1. **`__estalara_xid__`** — a 90-day cross-session identifier in `localStorage`, written only after
   consent is granted and erased on denial or withdrawal (§13.2). It is a random UUID, not a
   fingerprint. It is **not currently transmitted anywhere** (FOLLOW-146), so today it creates no
   server-side linkage; when FOLLOW-146 ships it will, by design and under the disclosure the
   shipped banner already carries.
2. **`lead_id`** — for an authenticated visitor, the first 16 hex characters of
   `SHA-256(Keycloak user UUID)` (`deriveLeadId()`), attached to chat and live events (`lead_id` in
   `packages/shared/src/schemas/events/{chat,live}.ts`). This is a deterministic, durable,
   cross-session key **by design** for registered users, and it is the clearest reason no absolute
   cross-session claim may be made anywhere in this corpus.
3. **Network and behavioural metadata** — the edge sees a source IP on every request, and the stored
   behavioural event data can support correlation independently of any identifier. Neither is
   addressed by the identifier's construction.

**Sessions minted before `d9160da0` do not have these properties.** Until 2026-08-24 the identifier
was an **unkeyed SHA-256 over four browser attributes** (`navigator.userAgent`, `screen.WxH`, the
`Intl` timezone and `navigator.language`) — a deterministic device digest, identical across tenants,
that did not rotate. Measured against production ClickHouse, real identifiers spanned 41–106 hours
across 2–5 calendar days, and because it was a device digest it **collided**: two visitors sharing a
device profile shared one identifier. Rows carrying those 64-hex identifiers remain in the event
stores until their retention expires; nothing re-derives or migrates them, and the ingest envelope
accepts both shapes. `events` carries a 13-month TTL
(`infra/clickhouse/migrations/0001_create_events.sql:46`), but `adaptation_decisions`, `llm_calls`
and `intent_events` carry **no TTL at all** (FOLLOW-1110 / FOLLOW-1111 / FOLLOW-1112), so for those
three tables "until retention expires" currently means "indefinitely". **Every statement in this
document about the identifier is therefore forward-dated: true of sessions minted at or after
`d9160da0`, false of sessions minted before it.** The residual risk this leaves is carried as Risk
A.

**Evidence for the historical description, re-runnable:**
`docs/compliance/FOLLOW-1105-session-identifier-assessment.md` §1, §2 and §12 (measurement,
read-only); `git show 852f5dee:packages/sdk/src/core/session.ts` (the original body);
`git log --all -S "day_bucket"` → documentation commits only, never code.

### 2.3 System Components

| Component                | Technology                                         | Function                                                                                           | Data Processed                                                                                                                                                                                                                                                 |
| ------------------------ | -------------------------------------------------- | -------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| JavaScript SDK           | TypeScript 5 + Preact 10, Shadow DOM               | Behavioral signal capture; mints and stores the session identifier (§2.2.1)                        | Behavioral signals (scroll depth, clicks, listing views, dwell time); a randomly minted UUID v4 session id. **No device attribute is read for identification** — no canvas, AudioContext, WebGL, screen geometry, timezone or language                         |
| Edge Ingest              | Cloudflare Workers (regions: fra/lhr/iad/dxb)      | Event validation, API-key auth (optional HMAC body signature, `apps/ingest/src/auth.ts`), batching | Behavioral event payloads, session_id (random UUID v4; historical rows carry the pre-`d9160da0` 64-hex digest — §2.2.1), tenant_id                                                                                                                             |
...
| DSR Queue                | Control Plane API + Postgres                       | Data subject request processing                                                                    | DSR type, session*id (the subject key), requester email (OTP delivery; retained only as a SHA-256 `email_hash` in `dsr_audit_log`), optional lead_id, jurisdiction. *`fingerprint_hash` was named here in error and exists nowhere in the code — FOLLOW-1107\_ |
...
2. SDK obtains the session identifier: it reads `sessionStorage['__estalara_session__']` and, only
   if that is empty, mints a fresh random UUID v4 and writes it there (§2.2.1). No browser attribute
   is read and no digest is computed. The identifier is stored client-side for the lifetime of the
   tab, and is disclosed as such in `PRIVACY_NOTICE_TEMPLATE.md` §4.
...
8. Adaptation decision is logged to ClickHouse (`adaptation_decisions` table, thirteen-month
   retention **declared but UNENFORCED — FOLLOW-1110**: the table carries no TTL in production, so
   rows currently persist indefinitely).
...
10. The browser clears `sessionStorage` — and with it the session identifier — when the tab is
    closed. **Correction (v2.20, FOLLOW-1107):** this step previously asserted a thirty-minute idle
    expiry releasing "session state" from a Redis cache. Neither half is verified at `d9160da0`: no
    idle timer expires the session identifier anywhere in the SDK, and the `session:{session_id}:*`
    Redis namespace has a **deleter** (the DSR erase path,
    `apps/control-plane/src/app/api/dsr/erase/route.ts`) but no writer anywhere in this repository
    (`grep -rn "'session:" apps packages` returns only that erase path). The thirty-minute constant
    that does exist is `INTENT_STATE_STALE_MS`, which bounds the intent-state entry described in
    §13.3, not the identifier.
...
| Session identifier (`session_id`, random UUID v4 — §2.2.1)                  | Browser `sessionStorage`; ClickHouse `events`; Postgres `session_embeddings` | Browser: tab lifetime (cleared by the browser on tab close). ClickHouse `events`: 13 months, enforced (`0001_create_events.sql:46`). Postgres `session_embeddings`: 90 days declared — **UNENFORCED (FOLLOW-1113)**, the nightly TTL cron this row assumed does not exist in `apps/control-plane/vercel.json` | Processing necessity; ePrivacy 5(3)(b) — the exemption argument is open, see §4 Risk E |
| Session embedding vector (1024-dim)                                         | Postgres: `session_embeddings`                                               | 90 days from last active event — **UNENFORCED (FOLLOW-1113)**: no TTL cron exists; rows are removed today only by the DSR erasure cascade                                                                                                                                                                     | LI — intent matching                                                                   |
...
| Archetype match and adaptation decisions                                    | ClickHouse: `adaptation_decisions`                                           | 13 months (AI Act audit trail per Master Design H.2) — **UNENFORCED (FOLLOW-1110)**: the table has no TTL in production, so retention is currently indefinite (over-retention)                                                                                                                                | Legal obligation + LI                                                                  |
| LLM call records                                                            | ClickHouse: `llm_calls`                                                      | 13 months — **UNENFORCED (FOLLOW-1111)**: the table has no TTL in production                                                                                                                                                                                                                                  | Audit, model improvement                                                               |
...
applications emit to Sentry and they are not treated alike. **FOLLOW-838 (2026-08-05) added a fifth
row: `apps/ingest` was missing from this table entirely, and it is the one app on the estate that
handles raw buyer chat.**
...
| `apps/control-plane` (Next.js)                                                  | **None** — no `beforeSend`, no scrubbing (`sendDefaultPii` explicitly `false`; decided FOLLOW-811, see §2.7.1)                                                                                                         | —                                                          |
| `apps/ingest` (Cloudflare Worker)                                               | **None** — no `beforeSend` (`observability.ts:71-82`; `sendDefaultPii` explicitly `false` since FOLLOW-845). Redaction is at the CALL SITES instead; see §2.7.2 (chat-NLP dispatch) and §2.7.3 (ClickHouse batch path) | —                                                          |
...
should get its own hook was decided under **FOLLOW-811**; see §2.7.1 immediately below.

### 2.7.1 FOLLOW-811 decision — no `beforeSend` for `apps/control-plane`; residual risk ACCEPTED with triggers (2026-08-05)

**Decision: no redaction hook was added.** The three configs (`sentry.server.config.ts`,
`sentry.client.config.ts`, `sentry.edge.config.ts`) still call `Sentry.init()` with no `beforeSend`.
Reasoning, in order of weight:

1. **There is nothing specified to redact, and the two available hook designs are both wrong here.**
   Mirroring the Python hook means gating on the `area=chat_intent` tag — a tag no capture site in
   this app emits, so the hook could never fire. That is a control that exists in the code and does
   nothing, which is the same class of defect as the "`beforeSend` with PII patterns regex" this
   very section had to retract. The alternative, a blanket scrub of the exception `value`, destroys
   triage: 95 non-test `Sentry.capture*` sites in `apps/control-plane/src` carry `err.message` as
   their entire diagnostic payload, against zero demonstrated buyer-text exposure.
2. **No PII-pattern regex was invented.** FOLLOW-811 AC(3) forbade it, for the reason recorded
   above: the correct response to an inaccurate record is to describe reality, not to build the
   fiction the record described.
3. **The load-bearing premise was re-verified, not inherited.** Re-grepped 2026-08-05:
   `src/app/api/adapt/route.ts:187-213`'s request schema carries no free-text field;
   `src/lib/chat-intent-cache.ts:145-166` parses a derived dimension map, and its Python source of
   truth (`apps/intent-engine/src/schemas.py:50-137`) has no raw-message field and confines
   `extraction_error` to `"<kind>: <ExceptionClassName>"`; the `extra`/`tags` payloads at the
   capture sites carry identifiers and status values only.

**What this assessment ADDS to the FOLLOW-739 record: the log sink and the Sentry sink are not
independent.** `consoleIntegration()` is a **default** integration of the installed Node SDK
(`@sentry/node-core@10.50.0`, `build/cjs/sdk/index.js:41`). Every `console.error`/`console.warn` in
`apps/control-plane` — 115 and 33 non-test call sites respectively — is therefore also a Sentry
input, arriving as a `category: "console"` breadcrumb on the next captured event; and the paragraph
above records that breadcrumbs are never modified on any path in any app. This is proven, not
inferred: `apps/control-plane/src/lib/__tests__/sentry-capture-path.test.ts` drives a real
`Sentry.captureException` through a real client with a memory transport and asserts a buyer-text
sentinel written to `console.error` is present in the outgoing envelope. **Consequence: a future
change that logs buyer-authored text discloses it to two sub-processors (Vercel and Sentry), not
one.**

**Scope correction (FOLLOW-838, 2026-08-05): this is NOT a property of `apps/control-plane`.** The
paragraph above was written from a single app and read for four months as if the other apps were
exempt. `consoleIntegration()` is a default of the **Cloudflare** SDK too —
`@sentry/cloudflare@10.50.0`, `build/cjs/sdk.js:29`, the last entry of `getDefaultIntegrations()`,
which `init()` applies whenever the caller passes no `integrations` key (`sdk.js:36-38`).
`apps/ingest/src/observability.ts:71-82` passes none. So the coupling holds in the ingest Worker as
well, and is proven there by the same technique
...
The implied negative for the remaining apps is likewise not to be inferred from silence: see §2.7.2
for `apps/ingest`, and for the Python tier the **explicit** finding that it is NOT affected on this
axis — `sentry_sdk`'s `LoggingIntegration` captures `logging` records, and every diagnostic line in
`apps/intent-engine` is a bare `print()`, which no Sentry integration observes. Modal's stdout log
(FOLLOW-812, and the "Modal application (stdout) logs" subsection of `ropa.md`) is therefore a
genuinely independent sink, not a duplicate of the Sentry one. That is recorded here so a future
reader does not have to re-derive it and does not assume symmetry with the two JS tiers.

**Vercel runtime-log retention is NOT RECORDED anywhere in this repository** — the Vercel
sub-processor row in `ropa.md` lists "API request logs" with no retention period, and no other
document states one. It is deliberately not guessed here. What the repo does establish, and what
bounds the exposure: **no log drain is configured** — `apps/control-plane/vercel.json` declares no
`logDrains`, and no drain reference exists in `infra/` — so these logs live only for Vercel's own
platform default and are not copied into any Time2Show-controlled long-lived store. Recording the
actual retention figure is a gap that belongs with the Vercel row in `ropa.md`, not here.

**Mitigating facts established while assessing** (all read from `@sentry/*@10.50.0` as installed,
not from vendor documentation):

- `sendDefaultPii` is now set **explicitly to `false`** in all three configs. This changes no
  behaviour — the SDK default is already falsy — but it pins the posture: `@sentry/core@10.50.0`
  `build/cjs/integrations/requestdata.js:7` carries
  `TODO(v11): Change defaults based on sendDefaultPii`, so a major bump could otherwise widen
  `user.ip` / request-header capture with no diff in this repo.
- `localVariablesIntegration()` is in the Node default list but is **inert**: it returns early
  unless `includeLocalVariables` is set, which it is not (`@sentry/node-core@10.50.0`,
  `build/cjs/integrations/local-variables/local-variables-sync.js:275`). Were it enabled, every
  local variable in scope at throw time would be attached to the event — a far wider surface than
  any exception message.
- Session Replay is wired in `sentry.client.config.ts` but **inert at the configured 0/0 sample
  rates**: `ReplayContainer.initializeSampling()` returns before creating a session or attaching any
  recorder (`@sentry-internal/replay@10.50.0`, `build/npm/cjs/index.js:8685-8691`). No DOM is read
  today. The capability, however, is one configuration line away, and it would record the admin
  dashboard's rendered DOM — a data category this DPIA does not cover. See trigger 2.

**Re-review triggers — any ONE of these requires this section to be re-assessed BEFORE the change
merges:**

1. Any buyer-authored free text (a chat message, a search string, a form field, an inquiry note)
   becomes readable inside `apps/control-plane`. This falsifies the premise outright rather than
   weakening it, and the decision above does not survive it.
2. `replaysSessionSampleRate` or `replaysOnErrorSampleRate` in `sentry.client.config.ts` is raised
   above `0`. Session Replay introduces rendered admin DOM as a new data category; this DPIA must be
   amended first, not afterwards.
3. `sendDefaultPii: true` or `includeLocalVariables: true` is set in any of the three configs.
4. A **major** `@sentry/nextjs` upgrade (10 → 11), for the `TODO(v11)` reason above.
5. `apps/control-plane` gains a capture site tagged `area=chat_intent`, or any capture site on a
   chat path — at that point the Python hook's design becomes applicable here and should be adopted
   rather than re-argued.
6. A mechanical gate for "raw exception message into an unscrubbed console/stdout sink" is extended
   to this app (FOLLOW-836). At that point "unscrubbed **by construction, not by control**" above
   stops being accurate and must be rewritten.

**Who checks the triggers:** whoever changes those files. The mechanical half of triggers 2 and 3 is
enforced by `apps/control-plane/src/lib/__tests__/sentry-config.shape.test.ts`, which asserts over
the whole init-options object and fails on any added or changed key; the redaction half is enforced
by `sentry-capture-path.test.ts`, which fails if a hook starts removing content. Triggers 1, 4, 5
and 6 have no mechanical enforcement and are review-time obligations.

**Time-based re-review:** at the next mandatory DPIA review (2027-05-15) or on any trigger above,
whichever is sooner.

### 2.7.2 FOLLOW-838 decision — `apps/ingest` chat-NLP dispatch redacted at the CALL SITE (2026-08-05)

`apps/ingest` is the only application on the estate that handles **raw buyer chat**. A
`chat.message.sent` event carries up to 4000 characters of buyer-authored text
(`packages/shared/src/schemas/events/chat.ts:40`), and
`apps/ingest/src/handlers/chat-nlp-dispatch.ts` forwards it to Modal as
`message: { role: 'user', content: <buyer text> }`. Until this ticket, a non-ok response from that
POST had **500 characters of the upstream response body** interpolated into a `console.error` line
and into the `Error` value handed to `Sentry.captureException` — i.e. into two Sentry fields at
once, per the scope correction in §2.7.1.

**What was established about the upstream body, by driving it rather than inferring it.** The
endpoint (`apps/intent-engine/src/main.py:115-167`) was exercised through its documented local
mirror (`local_dev.py`, same route signature, same FastAPI 0.139.0 / pydantic 2.13.3 the Modal image
resolves) with a buyer-text sentinel in `message.content`:

- Its four application-level rejection legs — 401 unauthorized, 400 missing-field, 400
  malformed-message, 400 blank tenant/session — return fixed strings and echo **nothing**.
- Its FastAPI/pydantic body-validation leg **does echo**: a `dict_type` failure returns
  `detail[0].input` containing the entire parsed request body, buyer message included. A
  `json_invalid` failure does not (`input` is `{}`).
- That echoing leg is **not reachable from the Worker's current request construction** (it always
  sends a JSON object), so **no live leak is claimed**. What is claimed is that the Worker's code
  did not depend on that: it copied whatever came back, unconditionally. The distance between "not
  reachable" and "reachable" is one body-shape change on either side — replacing
  `body: dict[str, Any]` with a Pydantic model, the obvious hardening of that endpoint, makes a
  merely missing field echo the whole body (driven: a `missing` error reports the parent object as
  `input`).
- **Not established:** Modal's own platform-level error bodies (gateway 5xx, container start
  failure). Driving those needs a deployed app and Modal credentials, neither available here. Their
  content is therefore unknown rather than assumed safe — which is a further argument for not
  copying the body at all.

**Decision: redact at the call site; no `beforeSend` hook.** The response body is no longer read.
The log line and the captured exception carry `HTTP <status> (<classified kind>)` — the same
"classified kind + identifier, never the text" convention the Python tier uses for
`extraction_error` (`nlp.py:504`, FOLLOW-812) — with `status` also on `tags`/`extra` for triage. No
PII-pattern regex was added (prohibited by FOLLOW-811 AC(3) and FOLLOW-838 AC(2), same reason: a
pattern matcher is a fiction that a record can then cite). A `beforeSend` hook was not added either,
for the FOLLOW-811 reason — a blanket scrub of the exception value would destroy triage across the
whole app to fix one call site — and because the call-site fix removes the data at source, which a
transport-layer hook cannot claim to do for the console breadcrumb of an unrelated future site.

**Explicitly accepted, not overlooked: the `.catch()` arm of the same function still carries
`err.message`.** That arm fires on a transport-layer rejection whose message the Workers runtime
generates (`Network connection lost.`, an `AbortError`, a `TypeError: Invalid URL`); no leg of it is
derived from the request body. Redacting it would cost the entire diagnostic on the most likely real
failure mode (Modal cold-start timeouts) for no established benefit. This is pinned executably:
`chat-nlp-sentry-capture-path.test.ts` asserts a sentinel planted in a rejection message **does**
reach the wire, so a future change in either direction surfaces there.

**The other diagnostic sites in `apps/ingest/src` were assessed too, not just the named one** (Rule
S; the full enumeration is in the FOLLOW-838 PR description). Summary of what carries what:

- `middleware/error-handler.ts:99`, `handlers/events.ts:450`, `:496`, `:655` — an unhandled
  `Error`'s `message`/`stack`. Not derived from any request body; same unscrubbed-**by construction,
  not by control** posture recorded for `apps/control-plane` in §2.7.1.
- `handlers/intent-snapshot.ts:390`, `:410`, `:446`, `:463` — these DO carry up to 300 characters of
  a ClickHouse / PostgREST error body, but the rows they answer for are explicitly PII-scrubbed
  (`intent-snapshot.ts:173-179`: signal counts, quiz leaf, archetype deltas, probabilities — no chat
  content), so no buyer free text is reachable through them.
- **One site outside this ticket's fix does carry the same shape as the one fixed:**
  `clickhouse-producer.ts:178` slices 500 characters of a ClickHouse error body into
  `CHPushFailure.error`, which `handlers/events.ts:592-600` puts into both a `logger.error` (→
  `console.log` → breadcrumb) and a `Sentry.captureException` value — and the batch being inserted
  contains `payload: JSON.stringify(event.payload)`, which for a `chat.message.sent` **is** the
  buyer's message. ClickHouse quotes the offending input in parse/format errors, so this one is not
  merely theoretical in the way the Modal 422 leg is. It is **not fixed here** because the redaction
  has a different cost — that body detail is the named diagnostic for the ESC-031 / F-02
  schema-drift failure mode and needs a replacement signal, not a deletion — and because widening
  this ticket into the events batch path would have taken it outside the chat handler it was scoped
  to. Filed as **FOLLOW-845** together with pinning `sendDefaultPii` explicitly in
  `observability.ts`. Until that lands, the disclosure this subsection removes from the chat-NLP
  dispatch path remains reachable through the ClickHouse batch path. **CLOSED 2026-08-06 by
  FOLLOW-845 — see §2.7.3, which also establishes by driving a real server that this path did echo
  buyer text, and that the echo was not a possibility but a routine property of ClickHouse parse
  errors.**

**Re-review triggers — any ONE requires this subsection to be re-assessed BEFORE the change
merges:**

1. Any code in `apps/ingest/src` logs or captures a value derived from an **upstream response body**
   or from an event `payload` field, on any path a `chat.message.sent` can reach.
2. `apps/intent-engine/src/main.py`'s `chat_nlp_endpoint` replaces `body: dict[str, Any]` with a
   Pydantic model, or the Worker starts sending a non-object body. Either makes the echoing 422 leg
   reachable, and the assessment above turns from prophylactic into remediating.
3. `apps/ingest/src/observability.ts` gains an `integrations` or `defaultIntegrations` key, or
   `sendDefaultPii: true` / `includeLocalVariables: true`. `sendDefaultPii: false` is pinned
   explicitly there as of FOLLOW-845 (2026-08-06), the way the three control-plane configs pin it;
   that was a no-op on the day it landed — `@sentry/cloudflare@10.50.0` `build/cjs/sdk.js:14` reads
   `options.sendDefaultPii ?? false` — and exists so the vendor's flagged `TODO(v11)` default change
   cannot flip it silently.
4. `SENTRY_DSN_INGEST` is provisioned. This path is inert today **by environment, not by control** —
   `observability.ts:72-75` returns `undefined` when the DSN is unset, and it is unset in production
   — so the operator action that sets it (instructed by FOLLOW-744, tracked as ESC-048) switches
   every sink in this subsection on at once.
5. A major `@sentry/cloudflare` upgrade (10 → 11), for the same `TODO(v11)` reason recorded in
   §2.7.1.

**Time-based re-review:** at the next mandatory DPIA review (2027-05-15) or on any trigger above,
whichever is sooner.

### 2.7.3 FOLLOW-845 decision — `apps/ingest` ClickHouse batch path: the response body is no longer read (2026-08-06)

This closes the item §2.7.2 assessed and deferred. `apps/ingest/src/clickhouse-producer.ts` POSTs a
batch of validated events to ClickHouse as NDJSON in which every row carries
`payload: JSON.stringify(event.payload)`; for a `chat.message.sent` that payload **is** the buyer's
message. On a non-ok response the producer sliced **500 characters of the response body** into
`CHPushFailure.error`, which `handlers/events.ts` and `handlers/events-retry-consumer.ts` each put
into a `logger.error` (→ `console.log` → Sentry breadcrumb, per §2.7.1) **and** into a
`Sentry.captureException` value.

**What was established, by driving a real server rather than inferring.** A
`clickhouse/clickhouse-server:25.8` container — the version `ci.yml` pins for both ClickHouse gates
— was loaded with the real `events` DDL (`infra/clickhouse/migrations/0001_create_events.sql`, local
`MergeTree` substitution) and fed the exact row shape `toClickHouseRow` produces, carrying a chat
sentinel in `payload`:

- ClickHouse quotes a ~200-character window of the **raw input**, anchored at the parse-failure
  position, in `Code: 26` / `Code: 27` / `Code: 117` errors. This is not an edge case; it is how the
  format parsers report failures.
- When the failure position falls at or after the row's trailing keys, that window **contains the
  buyer's chat text verbatim, inside the first 500 characters** — i.e. inside exactly the window
  that was being copied. Captured transcripts are in the FOLLOW-845 PR body; the exact captured body
  is the fixture in `apps/ingest/src/clickhouse-sentry-capture-path.test.ts`.
- A failure at an earlier key leaks a **partial** message instead (the window runs forward from the
  failure point into `payload`), and `Code: 117` quotes a 10-character fragment of the offending
  value directly. So the exposure is graded, not binary.
- Unlike the Modal 422 leg in §2.7.2, **no unreachability argument is available here**: this is the
  live post-ACK insert path for every batch, and the failure modes that produce these codes are the
  ordinary schema-drift ones the runbook expects.

**Decision: read the response HEADERS, never the body.** The failure descriptor is now built from
the HTTP status plus `X-ClickHouse-Exception-Code` (a bare integer the server generates, validated
as 1–5 digits before use) and a **local** code→class map, giving
`clickhouse_status_400:ch_code_27:row_rejected`. No upstream bytes are carried, and no PII-pattern
regex was added — same prohibition and same reason as §2.7.1/§2.7.2: the body is not sampled,
filtered or scrubbed, it is not read.

**The diagnostic is replaced, not deleted.** That body was the named signal for the ESC-031 / F-02
schema-drift failure mode, so deleting it would have traded a privacy defect for an observability
one. Two things replace it: the classified code above tells the operator _which class_ of failure
occurred, and `X-ClickHouse-Query-Id` — also header-derived, shape-validated as a UUID — is now
carried on both sinks so the operator can run
`SELECT exception FROM system.query_log WHERE query_id = '<id>'` and read ClickHouse's **full**
message, quoted input included. That is deliberate placement rather than a workaround: the detail
stays inside the store that already holds those rows by design (a controller-side system, in scope
of the same retention and DSR machinery), instead of being exported to a third-party processor whose
retention and access are governed separately. Whether chat text belongs in that table at all is a
different question, open as **ESC-049**, and is untouched by this decision.

**Explicitly accepted, not overlooked: the transport-rejection arm still carries `cause.message`.**
Symmetric with the `.catch()` acceptance in §2.7.2 and for the same reason — the message is
generated by the Workers runtime (`Network connection lost.`, an `AbortError` from the 4s
per-attempt timeout, `TypeError: Invalid URL`), no leg of it derives from the rows being POSTed, and
the ClickHouse credentials ride an `Authorization` header rather than the URL. Pinned executably: a
sentinel planted in a rejection message is asserted to **reach** the wire.
...
1. Any read of a ClickHouse (or other upstream) **response body** is reintroduced anywhere in
   ...
   slices in `handlers/intent-snapshot.ts`, which are safe today only because the rows they answer
   for are PII-scrubbed by construction (`intent-snapshot.ts:173-179`). If that scrubbing ever
   changes, those sites become instances of this same defect.
2. The producer starts sending a client-supplied `query_id`. The HTTP interface echoes it, which
   would turn a server-generated identifier into a caller-controlled string; the UUID shape check
   bounds that, and would need re-reviewing alongside.
3. A ClickHouse major upgrade changes the `X-ClickHouse-Exception-Code` header contract, or the
   error-code numbering the local class map encodes (the map's codes were resolved against 25.8 via
   `errorCodeToName`; an unmapped code degrades to `unclassified` plus the number, never to a body
   read).
4. `SENTRY_DSN_INGEST` is provisioned (ESC-048) — same trigger as §2.7.2 item 4: this whole path is
   inert today by environment, not by control.
...
| (b) | Reading investor chat        | Chat messages read to extract buying-intent signals; message text (PII-scrubbed for email/phone only) also retained in Adaptive-Listings' ClickHouse `events` table for 13 months by §H.8 design — corrected v2.15/FOLLOW-866, see §3.1 below |
...
**C-07 boundary (binding — must never be violated), corrected v2.15/FOLLOW-866 (ESC-049 CEO/DPO
ruling):** the boundary as previously stated here — "Raw chat text is stored APP-SIDE only …
Adaptive-Listings stores only the 12-dimensional intent vector … no free text, no message content" —
was verified only against the Redis shadow key path and was false as an ordinary reader would read
it. The corrected, binding boundary is: **no unscrubbed identifiers** (raw, un-redacted email
addresses or phone numbers) are stored by Adaptive-Listings at any layer. Within that boundary, two
things are both true and both shipped: (1) the Redis shadow key
(`shadow:{tenant_id}:{session_id}: chat_intent`) stores only the 12-dimensional intent vector, no
free text — verified in `schemas.py`, class `ChatIntentDetectedPayload` (no `messages` or `raw_text`
field) and `redis_writer.py` (`payload.model_dump()` serializes only `ChatIntentDetectedPayload`;
since FOLLOW-730 that payload also carries `data_source` and `extraction_error` — extraction
provenance, not buyer content); (2) separately, the buyer's chat message text — PII-scrubbed for
email/phone only, ≤4000 characters — IS retained in ClickHouse's `events` table (`payload` column)
for 13 months, by deliberate §H.8 design: emitted by the SDK
(`packages/sdk/src/index.ts:1518-1521`), scrubbed (`scrubMessagePii`,
`packages/sdk/src/core/pii-scrub.ts:22-27`), validated (`ChatMessageSentPayloadSchema.message`,
`packages/shared/src/schemas/events/chat.ts:39-40`), and written verbatim into `events.payload`
...
- Behavioral signals (scroll depth to certain sections, dwell time on price vs. floor plan, click
  ...
  session resolution.
- A random per-tab identifier held in `sessionStorage` (§2.2.1) is the minimum identifier needed to
  link events within a single visit. It is strictly less invasive than the alternatives on both axes
  that matter: unlike a cookie or a persistent local-storage token it does not survive the tab, and
  unlike a device fingerprint it reads nothing from the terminal equipment in order to identify the
  visitor. It is also less invasive than what this product shipped before 2026-08-24, which was a
  device fingerprint (§2.2.1).
...
- **Random, tab-scoped session identifier:** The identifier is minted at random and lives in
  `sessionStorage`, so a new browsing context starts from a new identifier that carries no
  computable relation to the previous one, and Time2Show holds no key or input with which to
  re-derive one from the other (§2.2.1). There is **no** thirty-minute idle rotation — no such timer
  exists in the SDK, and this bullet previously asserted one. What minimisation this delivers, and
  the three linkage vectors it does not close (`__estalara_xid__`, `lead_id`, network/behavioural
  metadata), are stated in §2.2.1; they are not restated here as guarantees.
...
### Risk A — Re-identification via Cross-Correlation of Stored Session Data

_Rewritten v2.20 (FOLLOW-1107). The pre-2.20 text assessed this risk on the premise of an
`HMAC(tenant_secret, entropy, day_bucket)` identifier that never existed. The risk itself did not go
away when the fingerprint did; it changed shape, and it now has a forward half and a historical
half._
...
session data with external data sources (IP geolocation, timing side-channels, the tenant's own
first-party records) to re-identify individual visitors.

**Forward half (sessions minted at or after `d9160da0`).** The identifier itself no longer carries
device entropy and is not re-derivable (§2.2.1), so the specific vector this risk was written about
— matching an Estalara identifier against a browser-fingerprinting database — is closed at the
identifier layer. What remains is correlation on the **payload**: behavioural sequences, listing
identifiers and timing, plus the source IP visible to the edge, plus the two deliberate durable keys
(`__estalara_xid__` once FOLLOW-146 ships, and `lead_id` for authenticated visitors). Nothing in the
identifier change reduces those.

**Historical half (rows minted before `d9160da0`), and it is the larger one today.** Every event row
written before 2026-08-24 carries the previous identifier: an unkeyed SHA-256 over four browser
attributes, offline-enumerable over a small input space (user-agent × resolution × timezone ×
language) and stable across visits and across tenants. For those rows the original risk statement
stands **without** the mitigations the pre-2.20 text claimed, because none of the three (tenant
secret, day bucket, non-derivability) ever existed. Those rows expire with their table's retention —
13 months for `events`, and **never** for `adaptation_decisions`, `llm_calls` and `intent_events`,
which have no TTL at all (FOLLOW-1110 / FOLLOW-1111 / FOLLOW-1112). **Enforcing those three TTLs is
therefore a re-identification control, not only a minimisation formality, and until they exist this
residual risk does not decay.**
...
- **Forward:** the identifier is minted at random and reads no device attribute, so it cannot be
  matched against any fingerprinting database or re-derived from a visitor's browser (§2.2.1,
  test-pinned).
- **Historical:** none of the three controls this list previously named ever existed. The only
  operative control over legacy rows is retention expiry, which is enforced for `events` and
  **unenforced** for `adaptation_decisions`, `llm_calls` and `intent_events` (FOLLOW-1110 / 1111 /
  1112). Tracked as an open residual, not as a mitigation.
...
**Residual Likelihood:** Low for sessions minted at or after `d9160da0`; **Medium** for the legacy
rows described above, and it does not fall until the three missing TTLs are enforced. **Residual
Severity:** Medium. **Residual Risk:** LOW-MEDIUM (GREEN-YELLOW band) forward; MEDIUM (YELLOW)
historical.
...
- LiteLLM router logs all LLM calls to the internal `llm_calls` ClickHouse table for auditability
  (thirteen-month retention **declared but UNENFORCED — FOLLOW-1111**: the table carries no TTL in
  production).
...
- Session identifiers are minted at random per tab (§2.2.1), so the same visitor on two tenant sites
  receives two unrelated identifiers and two different visitors are not assigned the same one. Note
  what this bullet no longer claims: cross-tenant isolation is **enforced** by the RLS policies,
  per-tenant partition keys and JWT tenant claims listed above — the identifier's randomness removes
  a collision hazard, it is not itself an access control. (The pre-2.20 text asserted a tenant
  secret in the identifier; there has never been one. Under the previous fingerprint the same
  visitor did receive an **identical** identifier on every tenant site, which is what FOLLOW-1108's
  cross-tenant DSR finding turned on.)
...
### Risk E — Regulatory Enforcement Action on Terminal-Equipment Storage and Access

**Description:** National supervisory authorities (CNIL, ICO, UAE Data Office, Polish UODO) may
determine that Mode A's storage of, and access to, a session identifier in the visitor's terminal
equipment does not qualify for the ePrivacy Directive Article 5(3)(b) strictly necessary exemption,
and may issue enforcement orders, fines, or require consent. The risk applies in two distinct
shapes: **forward**, to the random per-tab identifier described in §2.2.1, and **historically**, to
the device fingerprint the product used until 2026-08-24 — for which the exemption argument recorded
in this DPIA before v2.20 rested on properties that did not exist.
...
- **The factual predicate, stated without a legal conclusion.** Mode A stores one random identifier
  in tab-scoped `sessionStorage`, reads no device attribute, and does not persist beyond the tab
  (§2.2.1, test-pinned). Those are the facts on which an Art. 5(3)(b) argument would be built.
  **This DPIA does not assert that the exemption is available** — that is a legal question, isolated
  as question 4 in `docs/compliance/FOLLOW-1105-session-identifier-assessment.md` §9, and it is
  open. What would settle it: outside counsel's view on whether a per-session random token in
  `sessionStorage`, used to serve the interaction the visitor requested, falls inside 5(3)(b), read
  with EDPB Guidelines 02/2023 and the ICO's December 2024 fingerprinting position. Two facts are
  relevant to that view and are recorded here rather than argued: (i) until 2026-08-24 the
  identifier was a persistent cross-site device digest, for which the same question is materially
  harder and is question 1 of the same section; (ii) the shipped SDK asks for consent regardless —
  it renders the banner whenever consent is `pending` and fails closed if the consent text cannot be
  fetched — so the exemption is not, today, the operative control.
- Consent is obtained in practice by the shipped banner (see the Mode A correction in §2.2), which
  is the mitigation actually in force whatever the answer to the question above.
...
blocking the current hybrid pilot deployment because the shipped SDK obtains consent through the
banner in every mode (§2.2), so no processing currently depends on the strictly necessary argument
being available. **Correction (v2.20, FOLLOW-1107):** this paragraph previously concluded that "the
strictly necessary legal analysis is well-grounded in current ICO guidance". No such conclusion is
available from this document: the analysis it referred to rested on identifier properties that did
not exist, and the replacement question is open with counsel (§9 of the FOLLOW-1105 assessment,
questions 1 and 4). The external DPO must review this risk assessment before any scaling beyond the
...
| ---------------------------- | -------------------------------------------- | ----------------- | ---------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| A — Re-identification        | Low (forward) / Medium (pre-`d9160da0` rows) | Medium            | LOW-MEDIUM (GREEN-YELLOW) forward; MEDIUM (YELLOW) historical until FOLLOW-1110/1111/1112 land | No                                                                                       |
...
**ePrivacy:** Article 5(3) of Directive 2002/58/EC requires prior consent for storing information
in, or accessing information stored in, terminal equipment unless it is strictly necessary for a
service explicitly requested by the subscriber or user. The storage step in this product is the
session identifier written to `sessionStorage` plus the `localStorage` keys listed in
`PRIVACY_NOTICE_TEMPLATE.md` §4. Mode A is designed around the strictly necessary exemption
(§2.2.1); **whether that exemption is available is an open question with counsel, not a conclusion
of this document** (§4 Risk E; FOLLOW-1105 assessment §9 questions 1 and 4). In practice the shipped
SDK obtains banner consent in every mode, so no processing currently rests on the exemption. Mode B
requires consent collected by the tenant. The Estalara Consent Helper is available as a drop-in
component.
...
Directive, adopted 14 November 2023, finalised October 2024) confirm that Art. 5(3) is
technology-neutral: it reaches local storage and fingerprinting alike, not only cookies. Both limbs
matter to this record — the **forward** design engages the guidelines through its `sessionStorage`
and `localStorage` writes, and the **historical** identifier engaged them additionally as
fingerprinting (§2.2.1). The Estalara Mode A design has been assessed against these guidelines; the
strictly necessary analysis carried in the LIA (`docs/compliance/lia-template.md` §A.1,
TICKET-GDPR-003) was re-derived in v2.20 / FOLLOW-1107 because its previous form rested on a
mechanism that never existed, and its conclusion is now recorded as open pending counsel.
...
exemption) for storing or accessing information on a user's device. Mode A is designed around the
strictly necessary exemption in PECR Rule 6(4): the information stored is a single random session
identifier in tab-scoped `sessionStorage`, held solely to deliver the interactive listing experience
the visitor requested (§2.2.1). As with Art. 5(3)(b), **this document does not conclude that the
exemption is available** — see §4 Risk E. The word "fingerprinting" was removed from this paragraph
in v2.20: the product performs none, and the pre-2.20 sentence described the exemption as resting on
one.
...
exemption. **Correction (v2.20, FOLLOW-1107):** this paragraph previously stated that "the Estalara
Mode A falls within this characterization". That is a legal conclusion, it was drawn against an
identifier that did not exist, and it is withdrawn. The facts now available to whoever draws it are
in §2.2.1; the question is question 4 of the FOLLOW-1105 assessment §9. The ICO's online tracking
...
**Session identifiers as unique personal identifiers:** Under Cal. Civ. Code § 1798.140(ae), "unique
personal identifier" includes "a device identifier" and an "alias" persistently linked to a person,
and a browser fingerprint is squarely within it. Two statements follow, and they differ by date.
**Before `d9160da0`** the product's identifier _was_ a browser fingerprint (§2.2.1) and is treated
here as a unique personal identifier without qualification; rows carrying it are still held. **At
and after `d9160da0`** the identifier is a random per-tab token with no device input, which is a
weaker case for the definition — but this document does not rely on the difference: the identifier
is treated as a unique personal identifier for CCPA purposes either way, which is the conservative
reading and the one that keeps the GPC and deletion obligations below unqualified.
...
session identifiers linked to online behavior — the random session token of §2.2.1 as much as the
device digest that preceded it — constitute personal data as an "online identifier" under Article 1;
the classification does not turn on how the identifier is derived. Processing requires a lawful
basis (Article 5): explicit consent, contract performance, legal obligation, or protection of vital
interests. Specific references to UAE PDPL implementing regulations are verified with UAE-licensed
counsel.
...
**Withdrawal of the platform-registration consent — the direct, subject-facing channel (FOLLOW-710 /
FOLLOW-815, 2026-08-07).** Distinct from and in addition to the tenant-mediated workflow below: a
registered investor on app.estalara.com withdraws the §7 platform-registration consent by emailing
**compliance@estalara.com**, the address named in the §6.1 disclosure itself
(`PRIVACY_NOTICE_TEMPLATE.md`, `platform-v1.4-2026-08-07` and later) and rendered by
`renderPlatformConsentText()`. The subject does not need an account, a form, or the tenant's
cooperation to reach it — GDPR Art. 7(3) requires withdrawal to be as easy as granting, and granting
is one checkbox click. **This mailbox is an operator commitment: it must be monitored, and
withdrawal requests must be actioned inside the SLA below.** Texts served under
`platform-v1.3-2026-06-21` and earlier named no concrete address at all ("the agency's DSR contact")
— a defect recorded in `PRIVACY_NOTICE_TEMPLATE.md` §6.1 and closed by the same bump.

**DSR workflow (tenant-mediated — the path for embed-surface visitors and for Art. 15/17/20
requests):**
...
3. The tenant submits a DSR request to the Estalara DSR endpoint — **`POST /api/dsr/initiate`**
   (`apps/control-plane/src/app/api/dsr/initiate/route.ts`, Bearer JWT, tenant-scoped agency user;
   403 when the caller is not a tenant user) — with the verified requester proof, the DSR type, and
   the requester's details. The route validates a body of `{ session_id, email, dsr_type }` plus an
   optional `lead_id`. **`session_id` is the subject key** — `initiate` confirms it belongs to the
   calling tenant by looking it up in `session_embeddings`, and every downstream
   access/erase/portability query is keyed on it. `email` is used to deliver the verification OTP
   and is retained only as a SHA-256 hash in the ClickHouse `dsr_audit_log`
   (`apps/control-plane/src/app/api/dsr/_clickhouse.ts`); it is **not** a lookup key. The optional
   `lead_id` reaches CRM-written `conversion_labels` rows whose identifier namespace differs from
   `session_id`. _Corrected v2.20 / FOLLOW-1107: this step previously offered "session_id,
   fingerprint_hash, or hashed email" as interchangeable requester identifiers. `fingerprint_hash`
   appears nowhere in the codebase (`grep -rn "fingerprint_hash" apps packages` → no match), and an
   email hash is an audit field rather than something a requester can be looked up by — offering a
   tenant an intake identifier that resolves nothing is the same defect class this revision exists
   to remove._ **[CORRECTED 2026-08-07 — FOLLOW-710 AC-6 / FOLLOW-815, Rule AH.** This step named
   `POST /api/v1/dsr/request` from this document's creation until that date. **No such route has
   ever existed in the repo** (`find apps -ipath "*v1/dsr*"` returns nothing) — so the documented
   intake procedure named a phantom endpoint, and three consecutive retros (RETRO-228 §4a LG-2 among
   them) re-derived the same finding. The real route is and always was
...
- Engineering and architecture review: completed during Master Design v2.0 (2026-02 through
  2026-04). Privacy-by-design controls (k-anonymity, DP) were integrated into the system design from
  inception. _Corrected v2.20 / FOLLOW-1107: this list also named an "HMAC fingerprint" as a
  privacy-by-design control. No such control was ever built; the identifier that shipped from
  2026-05-10 to 2026-08-24 was an unkeyed device digest (§2.2.1), and the control that exists today
  is its removal (FOLLOW-1106)._
...
| 2.11a   | 2026-08-04 | Compliance Engineering | FOLLOW-739 (retroactively recorded 2026-08-05): §2.7 added — "Sentry redaction scope — partial, per-app, and no PII-pattern matching". Retracts the earlier claim that Sentry data is "scrubbed of PII via SDK `beforeSend` hook; CI-verified"; states per-app what actually runs. That edit landed without a revision row and the header was left at 2.10 while row 2.11 existed; both corrected in 2.12.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 2.12    | 2026-08-05 | Backend Engineering    | FOLLOW-811 (decision ticket): §2.7.1 added — the `apps/control-plane` Sentry residual risk is formally ACCEPTED, with reasoning, evidence and six named re-review triggers, and NO `beforeSend` is added (mirroring the Python hook would gate on an `area=chat_intent` tag this app never emits; a blanket scrub would destroy triage on 95 capture sites; no PII-pattern regex was invented, per the ticket's AC(3)). NEW FINDING recorded: the platform log sink and the Sentry sink are NOT independent in this app — `consoleIntegration()` is a default integration of `@sentry/node-core@10.50.0` (`build/cjs/sdk/index.js:41`), so its 115 `console.error` + 33 `console.warn` non-test call sites are also Sentry inputs as unmodified breadcrumbs; proven by a real-capture-path sentinel test, not inferred. Vercel runtime-log retention is recorded as NOT RECORDED in this repo and deliberately not guessed, bounded by the absence of any configured log drain. Code in the same PR: `sendDefaultPii: false` pinned explicitly in all three configs (no behaviour change; guards against the vendor's flagged v11 default change), the `sentry.client.config.ts` docstring corrected to name `NEXT_PUBLIC_SENTRY_DSN_CONTROL_PLANE`, the wrong "Capture unhandled promise rejections" comment on `replayIntegration()` corrected, and two tests pinning the decision. Header version corrected from a stale 2.10 (see row 2.11a). Log-sink gate for this app filed as FOLLOW-836.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
declared; **UNENFORCED — FOLLOW-1110**, so the trail is currently longer than declared, not shorter.
The art. 12 obligation is met; the art. 5(1)(e) storage-limitation statement about it is not).
...
a pseudonymous session token. No behavioral signals, no archetype inference, no adaptation. No
cross-tenant access; no export outside the EU unless the ClickHouse cluster is configured in a
non-EU region (documented separately in §4 of this DPIA).

**The token, corrected (v2.20, FOLLOW-1107).** The `consent.denied` event is dispatched from the
banner's `onDenied` path in `packages/sdk/src/index.ts`, which calls `getOrCreateSession()` in order
to attach a `session_id` to the audit event. Since FOLLOW-1106 that value is a random UUID v4
(§2.2.1). **Before `d9160da0` it was the visitor's device fingerprint**, so the audit record of a
refusal was keyed by a durable device identifier of the person who refused — the defect recorded as
§6.1 of the FOLLOW-1105 assessment and ticketed as FOLLOW-1109. This paragraph previously described
...
### 13.2 LIA — Cross-Session Identifier `__estalara_xid__` (Audit Finding F-14)

**Finding summary (audit):** The Estalara SDK offers cross-session continuity for tenants who opt in
to cross-listing journey tracking (§4.2 of this DPIA), by way of an identifier that persists for 90
days. The audit flagged that the consent banner copy did not explicitly disclose the cross-session
nature of the identifier, creating a gap between the actual behavior and the disclosed behavior.

**Processing activity — rewritten v2.20 (FOLLOW-1107); this closes the dual-narrative collision
tracked as FOLLOW-150.** The cross-session identifier is `__estalara_xid__`: a **randomly generated
UUID v4** with a creation timestamp, written to `localStorage` and treated as expired after 90 days.
It is **not** a fingerprint, not an HMAC, and not derived from any browser attribute. Verify against
`packages/sdk/src/core/session.ts`: `XSESSION_STORAGE_KEY = '__estalara_xid__'`;
`XID_TTL_MS = 90 * 24 * 60 * 60 * 1000`; `getOrCreateCrossSessionId()` (mints via `generateUuid()`,
rotates when `Date.now() - created_at > XID_TTL_MS`); `eraseCrossSessionId()` (`removeItem` plus
in-memory cache clear). `generateUuid()` uses `crypto.randomUUID()` where available and a
`Math.random`-based RFC-4122 v4 shape where it is not; the property this LIA relies on — that the
value is derived from nothing about the device — holds on both paths, while cryptographic
unpredictability holds only on the first.

_The pre-2.20 text described this identifier as "an HMAC of browser entropy attributes (canvas hash,
AudioContext hash, screen/viewport/timezone/language, WebGL renderer) keyed by `tenant_secret` and a
30-day rotation bucket". Every element of that sentence was false, and it contradicted §4 of the
Privacy Notice template — which has listed `__estalara_xid__` correctly as "UUID v4 + creation
timestamp" throughout. That contradiction is FOLLOW-150's "dual-id-narrative collision"
(MASTER_DESIGN:103, open since 2026-05-27); the UUID narrative is the true one and the HMAC
narrative is now deleted rather than reconciled._

**Transmission — the gap this LIA must not gloss.** The identifier is **written to `localStorage`
and never sent anywhere.** `getOrCreateCrossSessionId()` is called fire-and-forget at two points in
`packages/sdk/src/index.ts` (on init when consent is already granted, and in the `onGranted`
callback), and no SDK code path attaches its value to an event payload. The ingest side is already
half-ready — `apps/ingest/src/handlers/events.ts` reads an optional `cross_session_id` off
`intent.snapshot` events and forwards it to `handleIntentSnapshot()` — but nothing in
`packages/sdk/src` ever sets that field (`grep -rn "cross_session_id" packages/sdk/src` → no match).
The comment at the SDK call site says so explicitly: _"FOLLOW-146 will attach xid to event
payloads"_. **Consequence for this LIA: purposes (a), (b) and (c) below are not currently being
achieved by this identifier at all** — no cross-session linking happens server-side today. Until
FOLLOW-146 ships, this section describes a capability that is disclosed and consented but not
exercised. (Until `d9160da0` the cross-visit continuity the purposes describe was in fact being
provided by the _session_ identifier, because that identifier was a stable device digest —
undisclosed, and the inversion at the centre of the FOLLOW-1105 finding.)
...
2. **Necessity test — is processing necessary for that purpose?** Yes for purposes (a)–(c). A
   ...
   anonymous-visitor architecture. A 90-day random cross-session token is the minimum viable
   identifier for the stated purposes.

3. **Balancing test — does the legitimate interest override individual rights?** Balanced, with
   mitigations. The identifier is pseudonymous and carries no device or account input at all — it is
   a random UUID, so it cannot be re-derived from the visitor's browser and reveals nothing about
   the device by inspection. It is rotated after 90 days (limiting staleness), it is written only
   after consent is granted, and it is erased on explicit denial or withdrawal
   (`eraseCrossSessionId()`, called on both denial paths in `packages/sdk/src/index.ts`). However, a
   90-day cross-session identifier goes beyond what a typical visitor would expect without
   disclosure. The gap identified in audit F-14 is that the consent banner did not explain this;
   visitors therefore could not exercise meaningful informed objection. **The balancing test is
   passed only if the disclosure gap is remediated.** _Corrected v2.20 / FOLLOW-1107: this limb
   previously rested on the identifier being "not directly re-identifiable without the
   `tenant_secret`". There is no tenant secret and never was; the non-re-identifiability now claimed
   rests on randomness instead, which is a stronger and checkable basis._

   **Balancing test status: GREEN for the disclosure and erasure limbs.** FOLLOW-128 implemented the
   required disclosure strings for EN, PL and ES (landed on `main` 2026-05-27; the strings now ship
   out-of-bundle from `apps/control-plane/public/consent-text.json` per ADR-0021, with the byte
   record in `docs/compliance/consent-disclosures.canonical.json`). FOLLOW-139 implemented the
   matching SDK enforcement: `localStorage` storage with 90-day TTL rotation,
   `getOrCreateCrossSessionId()` called in `packages/sdk/src/index.ts` immediately after consent is
   granted (both on init when consent is already 'granted' and in the `onGranted` callback when the
   user accepts the banner), and `eraseCrossSessionId()` called on every consent-denied/withdrawal
   path in `packages/sdk/src/core/session.ts` and `packages/sdk/src/index.ts`. The disclosure gap is
   closed and the erasure-on-withdrawal obligation is implemented.

   _Qualified v2.20 (FOLLOW-1107): the word "unconditionally" is withdrawn._ What is verified is
   that the disclosed behaviour is not exceeded — the identifier is created only on consent, rotates
   at 90 days, and is erased on withdrawal. What is **not** verified is that the processing this
   test balances is happening at all: the identifier is never transmitted (FOLLOW-146, above), so
   the interests weighed on the controller's side are prospective. A balancing test whose
   controller-side interest is not yet being realised should be re-read, not reused, on the day
   FOLLOW-146 ships — and re-read again if the data flow it enables differs from purposes (a)–(c).

**Conclusion:** Processing is assessed as lawful under GDPR Art. 6(1)(f) (legitimate interests). The
interest is personalization continuity and conversion measurement. The legitimate interest is not
overridden by individual rights: the cross-session nature is disclosed (FOLLOW-128) and the 90-day
`localStorage` identifier with erasure-on-withdrawal is implemented and grep-verified (FOLLOW-139 —
`XID_TTL_MS`, `eraseCrossSessionId()`). The conclusion covers the identifier **as it exists at
`d9160da0`**: a random UUID that is stored in the visitor's browser and not transmitted. It does not
pre-approve the server-side linking that FOLLOW-146 would introduce.
...
`estalara_intent_{sessionId}`, where `sessionId` is the per-tab random session identifier described
in §2.2.1. The entry is a JSON envelope with schema:
...
2. **Server archetype (`session_embeddings` table, Postgres per-region):** Erased via the DSR
   ...
   Design §H.1.1. _Corrected v2.20 / FOLLOW-1107: this passage previously added that "the 13-month
   ClickHouse retention TTL enforces a bounded maximum lifetime without DSR action". That is true
   only of `events`; `adaptation_decisions`, `llm_calls`, `intent_events` and `session_quality`
   carry no TTL at all (FOLLOW-1110 / 1111 / 1112). For those tables the DSR cascade is currently
   the **only** deletion mechanism, which is the opposite of the reassurance the sentence offered._
...
`shadow:{tenant_id}:{session_id}:chat_intent` with a 24-hour TTL. No raw chat text is stored in this
Redis key (C-07 boundary — see §3.1 above, corrected v2.15/FOLLOW-866: the chat message text IS
separately retained in ClickHouse for 13 months by §H.8 design; this vector-scoped sentence is
unaffected by that correction — see §3.1 for the full boundary and the ClickHouse citation). The
derived vector is used exclusively to identify the investor's likely buying intent (purchase
purpose, urgency, budget band, family stage, geo priority, feature priority, cross-border status,
finance complexity, decision role, risk appetite, emotional state, tax awareness).
...
2. **Necessity test:** The 12-dim vector is the minimum data needed to drive archetype-prior
   updates. Raw chat messages are not needed for this computation (the vector's own necessity does
   not depend on the message text being retained). Corrected v2.15/FOLLOW-866: the message text IS
   separately retained in ClickHouse (§3.1) for a different purpose (§H.8) — that retention is not
   assessed by this necessity test, which is scoped to the vector only. The 24-hour TTL limits
   staleness. No less privacy-invasive alternative achieves the same intent-signal precision.

3. **Balancing test:**
   - The data is pseudonymous: the 12-dim vector is keyed by `session_id`, which since `d9160da0` is
     a random UUID carrying no device or account information (§2.2.1). It contains structured
     enumeration values, not free text. Re-identification from the vector alone would require
     joining it to another store on that key; the vector itself carries no identifying content.
     _Corrected v2.20 / FOLLOW-1107: this bullet called the key an "HMAC hash" and asserted
     re-identification was "technically infeasible". The first was never true. The second is
     restated as what can be checked — the vector's own content — rather than as an absolute about
     everything that could be joined to it._
...
retention windows for AL-owned data are: 12-dim intent vector 24 h — **enforced** natively by
Upstash Redis (`apps/intent-engine/src/redis_writer.py`, `ttl_seconds: int = 86400`); session
embeddings 90 days — **UNENFORCED (FOLLOW-1113)**, the nightly TTL cron does not exist in
`apps/control-plane/vercel.json`; adaptation decisions 13 months — **UNENFORCED (FOLLOW-1110)**, the
table has no TTL. _Corrected v2.20 / FOLLOW-1107: this sentence previously stated that all three
"are already enforced in shipped code". One of the three is._ They are re-asserted in ROPA Activity
16, which carries the same markers.
