...
| ↳ …caller OMITS `consent_text_hash`          | **201** — but for a PROVEN-external tenant the omission is already refused one step earlier with a `400` (FOLLOW-654 leg 2 / FOLLOW-660), so this row is reachable only for first-party or unattributable tenants. Since FOLLOW-707 the written default is the tenant's OWN rendered hash whenever its identity is provisioned; only a fallback identity falls back to `CANONICAL_CONSENT_TEXT_HASH` — itself a placeholder pending FOLLOW-704. Discouraged either way; the caller should GET-then-echo (`backlog/HANDOFFS.md` → FOLLOW-374, Step 1). |
