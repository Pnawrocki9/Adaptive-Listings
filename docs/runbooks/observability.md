...
| `adapt unresolved placeholder token` | a playbook directive carrying a `{token}` was DISCARDED before the response left `/api/adapt`, because neither the listing's own facts nor the shared server-resolvable contract could fill it — that slot was not adapted at all and the tenant's copy stands. `extra.unresolved_tokens` names every token that caused a drop; `tags.token` groups on the first. Level `warning`, but read it differently since the ESC-075 ruling: while twelve unsatisfiable tokens shipped this was steady state, whereas every token the playbooks ship is now server-resolvable, so on the playbook path a drop means a specific LISTING lacks a fact or a new unsourced token shipped. Worth reading, still not worth paging — the buyer sees the tenant's own copy either way. This is the SERVER end of the SDK's `adapt.skipped { reason: 'unresolved_token_<name>' }` (FOLLOW-1140 / ESC-074) | **none** |
