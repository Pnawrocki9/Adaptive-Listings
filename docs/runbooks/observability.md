...
| `adapt unresolved placeholder token` | a playbook directive carrying a `{token}` was DISCARDED before the response left `/api/adapt`, because neither the listing's own facts nor the shared server-resolvable contract could fill it — that slot was not adapted at all and the tenant's copy stands. `extra.unresolved_tokens` names every token that caused a drop; `tags.token` groups on the first. Level `warning`: for the twelve tokens ESC-075 records as unsatisfiable this is the system behaving correctly under a known gap, and paging on a known gap trains people to ignore pages. This is the SERVER end of the SDK's `adapt.skipped { reason: 'unresolved_token_<name>' }` (FOLLOW-1140 / ESC-074) | **none** |
