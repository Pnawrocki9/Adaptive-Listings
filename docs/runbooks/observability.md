...
Only these five carry a stable name, and all are cited **by name** in shipped documents — which is
why this register exists: those documents promised a Sentry event that could not be delivered. The
last row is the only `info`-level entry and the only one whose healthy state is a HIGH count — read
its row before reacting to it.
...
| `adapt unresolved placeholder token`  | a playbook directive carrying a `{token}` was DISCARDED before the response left `/api/adapt`, because neither the listing's own facts nor the shared server-resolvable contract could fill it — that slot was not adapted at all and the tenant's copy stands. `extra.unresolved_tokens` names every token that caused a drop; `tags.token` groups on the first. Level `warning`, and **not rare** — FOLLOW-1155 corrects the first ESC-075 pass on this row, which called it a regression signal. ESC-075 removed only the cause "no source for this token exists"; three routine causes remain, unmeasured: a request with no `listing_id` (drops every token-bearing directive at once), a non-OK listing-details response (**FOLLOW-1120 — open, P1, flapping**, so this signal cannot currently be distinguished from that outage), and an absent optional fact (`highlights?.[0]`, or a studio's `bedrooms: 0`, which drops the headline for four archetypes on every studio). Do not read a spike as a code regression before ruling those out. Still not worth paging — the buyer sees the tenant's own copy either way. This is the SERVER end of the SDK's `adapt.skipped { reason: 'unresolved_token_<name>' }` (FOLLOW-1140 / ESC-074) | **none** |
| `adapt ungrounded directive withheld` | every directive asserting a fact about the property was WITHHELD because this response came from a path that never read the listing — branch 2 (`similarity > HIGH_SIMILARITY_THRESHOLD`, which deliberately skips the listing fetch) or a `playbook_fallback_*` — so only the non-assertive `cta` was served. `extra.withheld_slots` names them; `tags.slot` groups on the first. Level `info` because this is **MASTER_DESIGN §E.7.0 working, not a failure**: a template cannot know a property, so it may not assert one. **READ THE COUNT BACKWARDS — a HIGH count is healthy and a count of ZERO is the anomaly**, since zero would mean the template paths stopped running altogether. Never page on it. If you are investigating a drop in adaptation rate, this is the expected cause and not a regression; the measured baseline and the three costs it carries are in ESC-077 (FOLLOW-1163 / ESC-076)                                                                                                                                                                                                                                                                                                                                    | **none** |

The other 102 sites build their message at runtime (`captureMessage(msg, …)`) or are
