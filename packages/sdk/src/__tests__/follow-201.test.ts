 ...
 *
 * Mirrors the exact logic in index.ts refreshDirectives() so unit tests validate
 * the algorithm without requiring the full init() lifecycle.
 */
...
describe('detectMismatch integration with applyQuizLeaf state (FOLLOW-201)', () => {
  ...
  it('quiz=flip_investor + investor behavioral → no mismatch (aligned)', () => {
    let behavioralState = initIntentState();
  ...
  it('mismatch.behavioral_archetype is the candidate tracked by the drift counter', () => {
    let behavioralState = initIntentState();
    for (let i = 0; i < 3; i++) {
      behavioralState = applyBehavioralSignal(behavioralState, 'listing.viewed');
    }
