...
function selfTest() {
  ...
  const undocumentedAddition = {
    ...sources,
    routeSrc: sources.routeSrc.replace(
  ...
  cases.push([
    'route-side undocumented addition (POST 201 + new code) FAILS',
    runCheck(undocumentedAddition).ok === false,
  ]);

  // (b) doc documents a status/code the route cannot emit — stale REMOVAL (doc still claims a
  //     code path that no longer exists in the route). Anchored on the POST block's row content
  //     (not on the sentinel/fence whitespace, which prettier is free to reflow) so this case
  //     survives markdown reformatting.
  const staleDocRemoval = {
    ...sources,
    docSrc: sources.docSrc.replace(
      '201\n400\n401\n409\n422 tos_version_superseded\n422 consent_text_hash_fabricated\n500',
      '201\n400\n401\n409\n422 tos_version_superseded\n422 consent_text_hash_fabricated\n500\n418 teapot_never_emitted',
    ),
  };
  cases.push([
    'doc-side stale/phantom entry (POST 418 never emitted) FAILS',
    runCheck(staleDocRemoval).ok === false,
  ]);

  // (c) GET/POST disjointness: the GET's coded 409 must NOT be treated as satisfying the POST's
  //     uncoded 409, and vice versa. Simulate by deleting the GET contract block's 409 line so
  //     ONLY the POST block still lists a 409 (uncoded) — this must still fail, because GET's
  //     409 must be documented in GET's OWN block, not "covered" by POST's 409 entry existing
  //     somewhere in the file.
  const disjointnessRegression = {
    ...sources,
    docSrc: sources.docSrc.replace(
  ...
  cases.push([
    ...
    runCheck(disjointnessRegression).ok === false &&
      // and it must fail for the GET-scoped reason specifically, not go unnoticed
      runCheck(disjointnessRegression).errors.some((e) => e.startsWith('GET ')),
  ]);

  // Sentinel-removal fail-closed check, same shape as check-consent-text-sync.mjs.
  const sentinelGone = {
    ...sources,
    docSrc: sources.docSrc.replace(
  ...
  cases.push(['removed GET BEGIN sentinel FAILS', runCheck(sentinelGone).ok === false]);
